...
def segment_image(im, gt, sigma, c,
                  min_size, num_ccs):
...
def main():
    if len(sys.argv) != 7:
        print("usage:", sys.argv[0], "sigma k min input output gt")
        return 1
    ...
    inputgt = cv2.imread(sys.argv[6])

    #print("processing")

    num_ccs = [];
    cv2.imwrite(sys.argv[5], segment_image(input, inputgt, sigma, k,
                                           min_size, num_ccs))
