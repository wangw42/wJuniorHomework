...
- (void)viewDidLoad{
    ...
    [self loadData:@"1" andEachPage:@"100"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self setLabel:cell.textContentLabel Detail:[_items[i] contentItem].detail];
 
    cell.timeLabel.text = [self timeStampToTime:[[_items[i] contentItem] PublishDate]];
     
    cell.likeNumberLabel.text = [NSString stringWithFormat:@"%d", [[_items[i] contentItem] likeNum]];
    cell.commentNumberLabel.text = [NSString stringWithFormat:@"%d", [[_items[i] contentItem] commentNum]];
    
    DataItem *contentItem = [_items[i] contentItem];
...
- (void)loadData:(NSString *)page
        andEachPage:(NSString *)eachPage {
    NSString *URL = @"http://172.18.178.56/api/content/public";
    NSDictionary *body = @{
        @"page":page,
        @"eachPage":eachPage,
    };
    ...
    [manager GET:URL parameters:body headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([response[@"State"] isEqualToString:@"success"])
        {
            ...
            for(int i = 0; i < n; i++)
            {
                UserNDataItem *newItem = [[UserNDataItem alloc]initWithDict:data[i]];
...
- (void)toUserPage:(UIButton *)btn {
    ...
    long i = indexPath.row;
    uvc.userID = [[self->_items[i] contentItem] ownID];
    uvc.userName = [[self->_items[i] userItem] userName];
...
- (void)showCommentPage:(UIButton *)btn {
    ...
    NSString *ownID = [[_items[indexPath.row] contentItem] ownID];
